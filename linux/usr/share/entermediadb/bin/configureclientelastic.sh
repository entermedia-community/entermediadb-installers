...
ln -s /usr/share/entermediadb/common/catalog/configuration $WEBAPP/$CLIENT_ID/catalog/configuration

mkdir -p $CATALOG/lists/searchtypes/

sed "s:%client_id:$CLIENT_ID:g;s:%client_name:$CLIENT_NAME:g;s:%sub_domain:$SUB_DOMAIN:g" < /usr/share/entermediadb/common/catalog/catalog/searchtypes/custom.xml >$CATALOG/lists/searchtypes/custom.xml
...
echo -e $outfile > /opt/entermediadb/webapp/$CLIENT_ID/emshare/_site.xconf
...
echo -e $outfile > /opt/entermediadb/webapp/$CLIENT_ID/mediadb/_site.xconf
...
echo -e $outfile > /opt/entermediadb/webapp/$CLIENT_ID/catalog/_site.xconf
